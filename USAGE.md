...
make test       # Runs the test suite (195 tests, all must pass)
```
...
| `auth_token` | (none) | Bearer token for API authentication (empty = no auth) |
...
Users and groups can be defined directly in `kerchunk.conf`, but it is recommended to use a separate `users.conf` file by setting `users_file = users.conf` in the `[general]` section. The web UI writes changes to this separate file, keeping the main config untouched.
...
1. **User override** -- `tx_ctcss` or `tx_dcs` set directly on the user
2. **Group fallback** -- `tx_ctcss` or `tx_dcs` set on the user's group
3. **Repeater default** -- `tx_ctcss` or `tx_dcs` set in the `[repeater]` section
...
Enable the web dashboard by setting `enabled = on` in the `[web]` section. Access it at `http://localhost:8080` (or your configured address/port).

If `auth_token` is set, the dashboard requires authentication. Add the token as a Bearer token in the Authorization header, or the dashboard will prompt for it.

For external access, set `bind = 0.0.0.0`. For HTTPS, provide `tls_cert` and `tls_key` paths pointing to PEM files.

#### Dashboard Page

The main dashboard (`/`) shows real-time repeater status:
...
- **PTT and COR status** -- whether the repeater is transmitting and whether a carrier is detected
- **Current caller** -- who is currently identified on the repeater
- **Queue status** -- how many audio items are waiting to play
- **Emergency mode indicator** -- shown when emergency mode is active
- **NWS alerts** -- active weather alerts for the local area
- **Active users** -- list of known users and their status
- **Event log** -- live scrolling log of repeater events via Server-Sent Events (SSE)
- **Controls** -- buttons to trigger CW ID, simulate DTMF commands, send TTS messages

#### Users Page

The users page (`/users.html`) provides user and group management:

- View all configured users and groups
- Add, edit, and delete users
- Modify user settings (name, identification codes, access level, group, voicemail, TOTP secret)
- Add, edit, and delete groups
- Changes are saved to the `users.conf` file
