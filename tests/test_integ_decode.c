...
void test_integ_decode(void)
{
    ...
    test_assert(t_dcs_detected == 1, "DCS not detected");
    /* DCS 047 is the inverted alias of 023 (same orbit).
     * Orbit canonicalization remaps to preferred label 023. */
    test_assert(t_dcs_code == 23 || t_dcs_code == 47, "wrong DCS code");
    test_end();
