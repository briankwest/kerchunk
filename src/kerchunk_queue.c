...
int kerchunk_queue_add_file(const char *path, int priority)
{
    ...
    item->id       = g_next_id++;
    snprintf(item->file.path, sizeof(item->file.path), "%s", path);

    insert_item(item);
    KERCHUNK_LOG_D(LOG_MOD, "queued file: %s (id=%d, pri=%d)", path, item->id, priority);
    return item->id;
}
...
int kerchunk_queue_add_buffer(const int16_t *buf, size_t n, int priority)
{
    ...
    insert_item(item);
    KERCHUNK_LOG_D(LOG_MOD, "queued buffer: %zu samples (id=%d, pri=%d)", n, item->id, priority);
    return item->id;
}
...
int kerchunk_queue_add_tone(int freq_hz, int duration_ms, int16_t amplitude, int priority)
{
    ...
    /* Pre-render the tone via libplcode (fixed-point sine LUT) */
    ...
    insert_item(item);
    KERCHUNK_LOG_D(LOG_MOD, "queued tone: %d Hz, %d ms (id=%d)", freq_hz, duration_ms, item->id);
    return item->id;
}
...
int kerchunk_queue_add_silence(int duration_ms, int priority)
{
    ...
    insert_item(item);
    KERCHUNK_LOG_D(LOG_MOD, "queued silence: %d ms (id=%d)", duration_ms, item->id);
    return item->id;
}
...
/* Load the next item for draining (iterative, not recursive) */
static int load_next_item(void)
{
    while (g_head) {
        ...
        if (item->type == QUEUE_AUDIO_FILE) {
            ...
            if (kerchunk_wav_read(item->file.path, &buf, &n, &rate) == 0) {
                g_drain_buf = buf;
                g_drain_len = n;
            } else if (kerchunk_pcm_read(item->file.path, &buf, &n) == 0) {
                g_drain_buf = buf;
                g_drain_len = n;
            } else {
                KERCHUNK_LOG_E(LOG_MOD, "failed to read audio file: %s", item->file.path);
                free_item(item);
