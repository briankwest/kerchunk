...
static int cmd_event(int argc, const char **argv, kerchunk_resp_t *r)
{
    ...
    static const char *names[] = {
        ...
        "QUEUE_DRAIN", "QUEUE_COMPLETE", "RECORDING_SAVED",
...
static void *audio_thread_fn(void *arg)
{
    ...
    while (g_running) {
        ...
        if (!queue_paused && !g_queue_ptt &&
            ...
            /* Skip TX delay if PTT is already held (e.g., by mod_repeater
             * during tail/hang — the radio is already keyed up) */
            if (kerchunk_core_get_ptt())
                g_tx_delay_rem = 0;
            else
        ...
        /* Drain queue audio (only after delay is exhausted) */
        int frames_drained = 0;
        if (!queue_paused && g_queue_ptt && g_tx_delay_rem <= 0) {
            while (frames_drained < 10) {
                if (kerchunk_core_get()->is_receiving() && !kerchunk_core_get_ptt())
                    break;
                if (kerchunk_audio_playback_writable() < KERCHUNK_FRAME_SAMPLES)
                    break;
                ...
                /* Dispatch to playback tap handlers (TX recording) */
                kerchevt_t play_evt = {
                    .type = KERCHEVT_AUDIO_FRAME,
        ...
        if (elapsed < AUDIO_TICK_US) {
            struct timespec ts = {
                .tv_sec  = 0,
                .tv_nsec = (long)((AUDIO_TICK_US - elapsed) * 1000),
            };
            nanosleep(&ts, NULL);
        }
    }
